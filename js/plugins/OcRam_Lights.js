...
"use strict"; if (!Imported || !Imported.OcRam_Core) alert('OcRam_Core.js ' +
    'is required!'); OcRam.addPlugin("Lights", "2.04");

/*:
 * @plugindesc v2.04 This plugin will add 1 layer for light sources. You may use event comments
 ...
 * @default {"LightType":"1","Radius":"400","Color":"#ffea00ff","OffsetX":"0","OffsetY":"0","Degrees":"360","Angle":"0","Rotation":"0"}
 *
 ...
 * *Comment* <light-ex:x,y:degrees:angle:rotation>
 *
 ...
 *
 * NOTE: If event has several 'light' comments on same event page first
 * occurance is used. Each event PAGE may include individual light source.
 * 
 ...
 *   battle_light_ex [x,y] [degrees] [angle] [rotation]
 *   Example: *Plugin Command* battle_light_ex 0,0 90 0 4
 * 
...
(function () {
    ...
    var _playerLightExParams = { offset: [_playerLightData.OffsetX, _playerLightData.OffsetY], degrees: _playerLightData.Degrees, angle: _playerLightData.Angle, rotation: _playerLightData.Rotation };
    ...
    var _battleLightExParams = { offset: [_battleLightData.OffsetX, _battleLightData.OffsetY], degrees: _battleLightData.Degrees, angle: _battleLightData.Angle, rotation: _battleLightData.Rotation };
    ...
    //var _debugMode = getBoolean(this.parameters['Debug mode']);
    ...
    var _terrainLightExParams = { offset: [0, 0], degrees: 0, angle: 0, rotation: 0 };
    ...
    Game_Follower.prototype.setupLightData = function (light_type, light_radius, light_color) {
        if (light_type != 0) {
            this._lightData = new Light_Data(this, light_type, light_color, parseInt(light_radius * 0.5), _playerLightExParams);
        } else {
    ...
    Game_Event.prototype.setupNewLightData = function () {
        ...
        if (cmts.length < 1) { // No light data
            ...
            //<light:[type:0-9]:[radius:1-1000]:[color:#rrggbbaa]:[degrees]>
            var ls_params = (cmts[0] + "::::").split(":");
            ls_params[1] = parseInt((ls_params[1]).replace(">", ""));
            ls_params[2] = parseInt((ls_params[2]).replace(">", ""));
            ls_params[3] = (ls_params[3]).replace(">", "");
            cmts = getCommentsOnActivePage(this, "<light-ex:");
            var ex_params = { offset: [0, 0], degrees: 0, angle: 0, rotation: 0 };
            if (cmts && cmts[0]) { // Extended parameters
                var tmp = (cmts[0] + ":::").split(":");
                tmp[1] = (tmp[1]).replace(">", "");
            ...
            } this._lightData = new Light_Data(this, parseInt(ls_params[1]), ls_params[3], parseInt(ls_params[2]) * 0.5, ex_params);
        }
    };
    ...
    Game_System.prototype.loadLightData = function () {
        ...
        if ($gameSystem._lightBGColor !== undefined) { // Do not break old saves...
            ...
            $gameMap._events.forEach(function (ev) {
                if (ev !== null) createNewLightData(ev, $gameSystem._eventLightData_OC[i++]);
            });
    ...
    Light_Data.prototype.initialize = function (parent, light_type, light_color, radius, ex_params) {
        ...
        this._currentColor = hexToRGBA(light_color); this._currentRadius = radius;
        ...
        if (this._lightExParams) {
            ...
            this._lightExParams = { offset: [0, 0], degrees: 0, angle: 0, rotation: 0 };
        }
        
        // Static stuff... updated only on page activation etc... 
        this._lightType = light_type; this._color = hexToRGBA(light_color);
    ...
    Light_Data.prototype.update = function (light_layer, ctx) {
        ...
        switch (this._lightType) {
            ...
            case _lightType.Pulsating_Slow: // Pulsating
                if (this._flickIsOn) {
            ...
            case _lightType.Pulsating_Fast: // Pulsating
                if (this._flickIsOn) {
        ...
        if (ctx !== null) {
            ...
            if (this._lightExParams.degrees == 0 || this._lightExParams.degrees == 360) { // "Normal" full circle light source
                ...
                } if (this._dir == 8 && obj.isPlayer_Lights_OC()) { // Fix for "flash light" effect
                    ctx.radialCone(this._x + this._lightExParams.offset[0], this._y + this._lightExParams.offset[1] - 24, this._currentRadius, this._currentColor, angle, this._lightExParams.degrees);
                } else {
                    ctx.radialCone(this._x + this._lightExParams.offset[0], this._y + this._lightExParams.offset[1], this._currentRadius, this._currentColor, angle, this._lightExParams.degrees);
                }
            }
        }
    ...
    OcRam_Light_Layer.prototype.updateMapLights = function () {
        ...
        $gameMap._events.forEach(function (ev) { // Update EVENT light sources
            if (ev !== null) { // We got valid event?
                if (ev._lightData) ev._lightData.update(class_ref, ctx);
            }
        });
    ...
    CanvasRenderingContext2D.prototype.radialCone = function (x1, y1, r1, c, angle, deg) {
    ...
    Game_Map.prototype.initTerrainLights = function () {
        ...
        if (_terrainLightSources.length > 0) return;
    ...
    function setIndoorsFlag() {
        if (!_useTimeSystem) return false;
    ...
    function createNewLightData(obj, light_data) { // Game_System saves only data NOT actual references... Need to re-attach 'em!
        if (obj) {
            if (light_data === null) {
                ...
                obj._lightData = new Light_Data(obj, light_data._lightType, RGBAToHex(light_data._color), light_data._radius, light_data._lightExParams);
    ...
    function hexToRGBA(p) {
        ...
        return result ? {
            ...
            g: parseInt(result[2], 16),
            b: parseInt(result[3], 16),
            a: parseFloat(parseInt(result[4], 16) / 255).toFixed(2)
        } : null;
    }
